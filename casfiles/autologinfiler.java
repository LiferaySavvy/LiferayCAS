...
public class CASAutoLogin implements AutoLogin 
{
	...
	public String[] login(HttpServletRequest request, HttpServletResponse response) 
	{
		...
		System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		System.out.println(" CASAutoLogin class login method From Ext.!!!");
		System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
		...
		try 
		{
			...
			if (PrefsPropsUtil.getBoolean(companyId, PropsKeys.CAS_IMPORT_FROM_LDAP,PropsValues.CAS_IMPORT_FROM_LDAP)) 
			{
				try 
				{
					...
					{
						user = importLDAPUser(companyId, StringPool.BLANK, login);
					}
					else 
					{
						user = importLDAPUser(companyId, login, StringPool.BLANK);
					}
				}
			...
			/**
			 * The following code added by Vidyayug in order to check whether user is belonging to the website or not. If user is belonging to 
			 * site then we will allow him to login into the website otherwise we will navigate user to login page.
			 */
				...
				if(group.isOrganization())
				{
					 ...
					 try 
					 {
							...
							for(Organization organization:userOrganizations)
							{
								...
								Group group1 = GroupLocalServiceUtil.getOrganizationGroup(companyId,organizationId);
