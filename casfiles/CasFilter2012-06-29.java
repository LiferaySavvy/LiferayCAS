...
public class CASFilter extends BasePortalFilter {
	...
	protected void processFilter(HttpServletRequest request,
		...
		// System.out.println("#####################::serverURL before"+serverURL);
		if (serverURL.indexOf("/portal/logout") != -1) {

			serverURL = StringUtil.replace(serverURL, "/c/portal/logout", " ");
		}
		if (serverURL.indexOf("/portal/login") != -1) {
			serverURL = StringUtil.replace(serverURL, "/c/portal/login", " ");
		}
		...
		if (PrefsPropsUtil.getBoolean(companyId, PropsKeys.CAS_AUTH_ENABLED,
			...
			if (pathInfo.indexOf("/portal/logout") != -1) {
				...
				logoutUrl = HttpUtil.addParameter(logoutUrl, "logoutUrl",
						serverURL);
				...
				if (Validator.isNull(serviceUrl)) {
					...
					serviceUrl = CommonUtils.constructServiceUrl(request,
							response, serviceUrl, serverURL, "ticket", false);
				...
				if (Validator.isNull(ticket)) {
					if (Validator.isNotNull(login)) {
						...
						loginUrl = HttpUtil.addParameter(loginUrl, "service",
								serviceUrl);
